...
class RecurrentWindowAttention(nn.Module):
    ...
    def __init__(
        ...
        self.relative_position_bias_table_cross_s = nn.Parameter(
            torch.zeros((2 * window_size[0] - 1) * (2 * window_size[1] - 1), num_heads // 2)
        )  # 2*Wh-1 * 2*Ww-1, nH
        ...
        self.relative_position_bias_table_cross_s = trunc_normal_(self.relative_position_bias_table_cross_s, std=0.02)
        self.relative_position_bias_table_self_s = trunc_normal_(self.relative_position_bias_table_self_s, std=0.02)
        self.relative_position_bias_table_cross_x = trunc_normal_(self.relative_position_bias_table_cross_x, std=0.02)
        self.relative_position_bias_table_self_x = trunc_normal_(self.relative_position_bias_table_self_x, std=0.02)
    ...
    def forward(self, input_x, state_x, mask=None):
        ...
        state_linear = (
            self.state_linear(state_x).reshape(B_, N, 4, self.num_heads // 2, C // self.num_heads).permute(2, 0, 3, 1, 4)
        )
        embed_linear = (
            self.embed_linear(input_x).reshape(B_, N, 4, self.num_heads // 2, C // self.num_heads).permute(2, 0, 3, 1, 4)
        )
    ...
    def forward(self, x, state, x_size, x_size_next):
        ...
        if self.gate == "cat":
            if self.input_resolution == x_size:
                attn_windows, state_windows = self.attn(
                    x_windows, state_windows, mask=self.attn_mask
            ...
            if self.input_resolution == x_size:
                output_x, (state_x, reset, forget) = self.attn(
                    x_windows, state_windows, mask=self.attn_mask
                )  # nW*B, window_size*window_size, C
            else:
                output_x, (state_x, reset, forget) = self.attn(
                    x_windows, state_windows, mask=self.calculate_mask(x_size).to(x.device)
                )
            ...
            if self.input_resolution == x_size:
                output_x, (self_s, cross_s) = self.attn(
                    x_windows, state_windows, mask=self.attn_mask
                )  # nW*B, window_size*window_size, C
            else:
            ...
            if self.input_resolution == x_size:
                output_x, (main, input, forget) = self.attn(
                    x_windows, state_windows, mask=self.attn_mask
                )  # nW*B, window_size*window_size, C
            else:
                output_x, (main, input, forget) = self.attn(
    ...
    def __init__(
        ...
        if resi_connection == "1conv":
            ...
            self.conv = nn.Sequential(
                nn.Conv3d(dim_next, dim_next // 4, 3, 1, 1),
...
class ssrt(nn.Module):
        ...
        **kwargs,
        ...
        for i_layer in range(half_layers, self.num_layers):
            patches_resolution_list.append(
                (
                    patches_resolution[0] // (2 ** ((self.num_layers - i_layer) - 1)),
    ...
    def forward_features(self, x_list):
        ...
        for i_layer in range(self.half_layers, self.num_layers):
            x_list_, x_list_res, x_size_next = self.layers[i_layer](
                add_list(x_list_, res_group[self.num_layers - i_layer - 1]), x_size
            )
            x_size = x_size_next
        x_list__ = []
        for i in range(len(x_list)):
